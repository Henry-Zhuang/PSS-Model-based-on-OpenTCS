...
public interface InternalVehicleService
   ...
   * @param vehicleRef A reference to the vehicle to drop the bin.
   * @param location A destination location where the bin is placed.
   * @throws ObjectUnknownException 
   */
  void pushBinToLocation(TCSObjectReference<Vehicle> vehicleRef, Location location)
      throws ObjectUnknownException;
  /**
   * Get the locations from the globalObjectPool by the given references.
   * This function is used in LoopbackCommunicationAdapter.canProcess().
   * @param opDestinations The given destination references
   * @return 
   */
  default List<TCSObject<?>> getDestinations(List<TCSObjectReference<?>> opDestinations){
    return null;
  }
  ////////////////////////////////////////////////////////////////////////modified end
}
