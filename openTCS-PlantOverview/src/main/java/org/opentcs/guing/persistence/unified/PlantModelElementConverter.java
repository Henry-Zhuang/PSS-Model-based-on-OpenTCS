...
public class PlantModelElementConverter {
  ...
  public LocationModel importLocation(LocationCreationTO locationTO,
    ...
    model.getPropertyModelPositionY().setValueAndUnit(locationTO.getPosition().getY(),
                                                      LengthProperty.Unit.MM);
    model.getPropertyModelPositionZ().setValueAndUnit(locationTO.getPosition().getZ(),
                                                      LengthProperty.Unit.MM);
    ...
    // 注意LocationModel有7个属性,而LocationCreationTO不一定有7个料箱
    while(i<model.getBinsSize()){
        model.getPropertyBin(i).get(0).setText("");
        model.getPropertyBin(i).get(1).setText("");
        i++;
    }
    model.getPropertyBinsNum().setValue(locationTO.getBins().size());
    /////////////////////////////////////////////////////////////////modified end
