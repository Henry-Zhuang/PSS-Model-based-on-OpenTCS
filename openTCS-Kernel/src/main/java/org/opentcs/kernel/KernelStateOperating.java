...
class KernelStateOperating
  ...
  public void initialize() {
    ...
    for (Vehicle curVehicle : getTCSObjects(Vehicle.class)) {
      setVehicleProcState(curVehicle.getReference(), Vehicle.ProcState.IDLE);// modified by Henry ÐÞ¸Ä³µÁ¾³õÊ¼×´Ì¬
      vehicleService.updateVehicleIntegrationLevel(curVehicle.getReference(),
                                                   Vehicle.IntegrationLevel.TO_BE_UTILIZED);// modified by Henry ÐÞ¸Ä³µÁ¾³õÊ¼×´Ì¬
      setVehicleState(curVehicle.getReference(), Vehicle.State.IDLE);// modified by Henry ÐÞ¸Ä³µÁ¾³õÊ¼×´Ì¬
