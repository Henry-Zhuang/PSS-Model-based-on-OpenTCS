...
class KernelStateOperating
  ...
  public void initialize() {
    ...
    for (Vehicle curVehicle : getTCSObjects(Vehicle.class)) {
      setVehicleProcState(curVehicle.getReference(), Vehicle.ProcState.IDLE);// modified by Henry 修改车辆初始状态
      vehicleService.updateVehicleIntegrationLevel(curVehicle.getReference(),
                                                   Vehicle.IntegrationLevel.TO_BE_UTILIZED);// modified by Henry 修改车辆初始状态
      setVehicleState(curVehicle.getReference(), Vehicle.State.IDLE);// modified by Henry 修改车辆初始状态
