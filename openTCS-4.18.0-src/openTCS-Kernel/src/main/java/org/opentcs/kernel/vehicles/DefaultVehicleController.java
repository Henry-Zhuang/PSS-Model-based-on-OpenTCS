...
public class DefaultVehicleController
  ...
  private volatile boolean isTrackVehicleInPlace;
  
  private volatile boolean isBinVehicleInPlace;
  //////////////////////////////////////////////// modified by Henry
  /**
  ...
  public DefaultVehicleController(@Assisted @Nonnull Vehicle vehicle,
                                  ...
                                  @Nonnull ChangeTrackService changeTrackService,
  ...
  public void setDriveOrder(@Nonnull DriveOrder newOrder,
    ...
    synchronized (commAdapter) {
      ...
      if(vehicle.getType().equals(Vehicle.TRACK_VEHICLE_TYPE))
        scheduler.claim(this, asResourceSequencePST(newOrder.getRoute().getSteps()));
      else
  ...
  @Override
  @Nonnull
  public ExplainedBoolean canProcess(@Nonnull List<String> operations, @Nonnull List<TCSObjectReference<?>> opDestinations) {//modified by Henry
    requireNonNull(operations, "operations");

    synchronized (commAdapter) {
      return commAdapter.canProcess(operations, opDestinations);
    }
  }
  ...
  private void updatePositionWithoutOrder(Point point) {
    ...
    if (point != null && !vehicle.getType().equals(Vehicle.TRACK_VEHICLE_TYPE)) {// TEST
      try {
  ...
  public void setTrackVehicleInPlace() {
    synchronized(DefaultVehicleController.this){
      if(vehicle.getType().equals(Vehicle.TRACK_VEHICLE_TYPE))
        return;
      this.isTrackVehicleInPlace = true;
