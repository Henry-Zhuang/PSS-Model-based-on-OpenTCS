...
public class LoopbackCommunicationAdapter
  ...
  @Override
  public synchronized ExplainedBoolean canProcess(List<String> operations, List<TCSObjectReference<?>> opDestinations) {//modified by Henry
    requireNonNull(operations, "operations");
    requireNonNull(opDestinations, "opLocations");
    ...
    Iterator<TCSObject<?>> locIter = vehicleService.getDestinations(opDestinations).iterator();// modified by Henry
    Map<String,Integer> stackSize = new HashMap<>();
    for(TCSObject<?> destination:vehicleService.getDestinations(opDestinations)){
      if(destination instanceof Location){
        Location location = (Location)destination;
        stackSize.put(location.getName(), location.stackSize());
      }
    }
    while (canProcess && opIter.hasNext()) {
      final String nextOp = opIter.next();
      final TCSObject<?> nextLoc = locIter.next();
      int tmp;
      ...
      if (loaded) {
        if (nextOp.startsWith(getProcessModel().getLoadOperation())) {
          ...
          if(stackSize.get(nextLoc.getName()).equals(Location.BINS_MAX_NUM)){
            canProcess = false;
            reason = PUSH_FULL_STACK;
          }
          else{
            tmp = stackSize.get(nextLoc.getName()) + 1;
            stackSize.put(nextLoc.getName(), tmp);
            loaded = false;
          }
        }
      } // If we're not loaded, we could load, but not unload.
      else if (nextOp.startsWith(getProcessModel().getLoadOperation())) {
        if(stackSize.get(nextLoc.getName()).equals(0)){
            canProcess = false;
            reason = POP_EMPTY_STACK;
        }
        else{
          tmp = stackSize.get(nextLoc.getName()) - 1;
          stackSize.put(nextLoc.getName(), tmp);
  ...
  private class VehicleSimulationTask
    ...
    protected void runActualTask() {
      ...
      if (curCommand == null) {
        ...
        if (!curCommand.isWithoutOperation()) {
          simulateOperation(curCommand.getOperation(),curCommand.getOpLocation());
        }
     ...
     */
    private void simulateOperation(String operation, Location opLocation) {
...
//      else
      ...
      if (operation.equals(getProcessModel().getLoadOperation()) && opLocation.stackSize()!=0) {
        ...
        //modified by Henry
        // if "Unload", push Vehicle's workBin to the Location
        vehicleService.pushBinToLocation(vehicle.getReference(),opLocation);
